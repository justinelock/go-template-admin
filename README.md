...
<h4 align="center">基于 Go-Zero 的若依风格总控后端，支持多租户、RBAC 与本地 Helm 部署</h4>
...
- 快速搭建企业级后台管理系统后端。
- 学习 Go-Zero 项目组织与分层方式。
- 基于若依生态前端快速对接后端接口。
- 在本地 k3d/Kubernetes 中验证部署拓扑。
...
```text
前端 /api → Traefik :28080 → system :8086
                              ├── /auth/*     登录、验证码、租户列表
                              ├── /system/*   系统管理（用户/角色/菜单…）
                              └── /member/* …  业务总控 API
```

- **总控单体**：`system` 单进程提供登录与全部管理端 API，无需 etcd、无需独立 auth 服务。
- **demo**：可选示例模块（`:8099`）。
- 对外 URL 与若依前端约定一致，前端无需因单体化改接口路径。

## 服务模块

| 模块 | 路径 | 说明 |
| --- | --- | --- |
| system | `app/system` | 登录（`/auth`）、用户/角色/菜单、租户、业务总控 API 等 |
...
- [Helm Chart 说明](deploy/k3d/docs/HELM_CHART_K3D.md)
- [Redis 安装部署文档](deploy/helm/ovra-zero/INSTALL_REDIS.md)
- [Docker 重启后恢复手册](deploy/k3d/docs/RECOVERY_AFTER_DOCKER_RESTART.md)
- [Helm Chart README](deploy/helm/ovra-zero/README.md)

其中 `INSTALL_REDIS.md` 包含 Kubernetes 与 Linux 裸机 Redis Cluster 部署说明。
...
- [x] 总控单体化（auth 合并 system，无需 etcd）
- [ ] 报表大屏可视化
